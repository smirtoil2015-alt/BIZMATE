...
export default function AlertsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const pendingByAlert = new Set(approvalSnap.docs.map((item) => String(item.data().sourceAlertId ?? '')).filter(Boolean));
        if (!cancelled) {
          setOrganizationId(orgId);
          setCompany(orgName);
          setAlerts(buildAlerts(data.insights, data.projects));
          setQueued(Object.fromEntries([...pendingByAlert].map((id) => [id, true])));
        }
      } catch (err) {
  ...
  async function queueCriticalAction(alert: SmartAlert) {
    if (!user || !organizationId || alert.priority !== 'critical' || queued[alert.id]) return;
    setError('');
    setNotice('');
    try {
      ...
      setNotice('Critical alert sent to Action Center for owner approval.');
    } catch (err) {
  ...
  return <main className="module-page">
    ...
    <section className="data-card"><div className="data-head"><h2>What needs attention</h2><span>{alerts.length ? 'Ranked by urgency' : 'No active alerts'}</span></div>
      <div className="insight-grid">{alerts.map((alert) => <article className={`insight-card ${alert.priority === 'critical' ? 'critical' : alert.priority === 'high' ? 'warning' : alert.priority === 'positive' ? 'opportunity' : ''}`} key={alert.id}><div><span>{alert.priority.toUpperCase()}</span><h3>{alert.title}</h3><p>{alert.description}</p></div>{alert.metric && <strong>{alert.metric}</strong>}<small>{alert.source}</small>{alert.priority === 'critical' && <button className="secondary" onClick={() => void queueCriticalAction(alert)} disabled={Boolean(queued[alert.id])}>{queued[alert.id] ? 'Approval requested ✓' : 'Create approval →'}</button>}</article>)}{!alerts.length && <div className="module-empty compact"><h3>Everything is quiet.</h3><p>No critical risks or active signals were found in the current company data.</p></div>}</div>
    </section>
    <section className="data-card"><div className="data-head"><h2>How BIZMATE prioritizes</h2></div><p className="brief">Critical project risks and critical business insights rise to the top. Warnings follow, while opportunities remain visible without being confused with confirmed risks. BIZMATE queues critical actions for explicit owner approval instead of executing them automatically.</p></section>
  </main>;
}
