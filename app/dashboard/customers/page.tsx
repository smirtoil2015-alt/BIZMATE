...
export default function CustomersPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) { setOrganizationId(orgId); setCustomers(rows); }
      } catch (err) { if (!cancelled) setError(err instanceof Error ? err.message : 'Unable to load customers.'); }
      finally { if (!cancelled) setBusy(false); }
    }
    void load(); return () => { cancelled = true; };
  }, [user]);

  const filtered = useMemo(() => { const q = search.trim().toLowerCase(); if (!q) return customers; return customers.filter((c) => `${c.name} ${c.company ?? ''} ${c.email ?? ''}`.toLowerCase().includes(q)); }, [customers, search]);
  ...
  async function addCustomer(event: FormEvent) {
    event.preventDefault(); if (!organizationId || !user || !form.name.trim()) return;
    setSaving(true); setError('');
    try {
      const value = Number(form.value || 0);
      const data: Omit<Customer, 'id'> = { organizationId, name: form.name.trim(), company: form.company.trim() || undefined, email: form.email.trim() || undefined, status: form.status, value: Number.isFinite(value) ? value : 0, lastContactAt: new Date().toISOString().slice(0, 10) };
      ...
      await runAutomations(organizationId, 'customer.created', { customerId: id, customerName: data.name, company: data.company ?? '', actorId: user.uid });
